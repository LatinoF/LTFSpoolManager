...
- **Cloud Backup**: Automatic daily backup to Nextcloud via WebDAV (files named `YYYY.MM.DD_LTF_SpoolManager_DB.json`), with folder browsing and one-click restore. Credentials are stored only in the browser.
...
- **Data**: Browser `LocalStorage` for persistent storage, with optional daily auto-backup to Nextcloud (WebDAV).
...
5. **Backup**: Use the settings menu (gear icon) to export your database, or configure a Nextcloud WebDAV folder for automatic daily cloud backups.

> **Nextcloud setup**: add your site's domain to `config.php` via `'cors.allowed-domains' => ['https://<your-domain>']` and use an app password (not your account password) in the app settings.
